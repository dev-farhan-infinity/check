import { p as propTypes$1, t as tagPropType, m as mapToCssModules, c as classnames, j as jsx, r as react, x as jsxs, z as useSkin, F as Fragment, L as useDispatch, N as useSelector, B as Button, av as emptyGetVideo, aw as getCreateVideo, ax as postCreateAvatar, ay as postCreateVideo, az as createAvatarLanguage, P as subscriptionActions, aA as getCreateAvatar, aB as emptyGetAvatar } from "./index.b27a7f54.js";
import { C as Container } from "./Container.9d0ff769.js";
import { b as CardBody, I as Input, L as Label, h as FormFeedback, _ as __awaiter, H as __generator, J as __spreadArray, K as __read, R as Row, r as Col, M as StateManagedSelect$1, N as selectThemeColors, O as components, P as Cleave, a as create, Q as create$1, T as create$2, u as useFormik, c as create$3, B as BreadCrumbs, F as Form, U as Nav, V as NavLink, g as CardText, W as TabContent, X as TabPane, y as CardHeader, S as Spinner$1, Y as InputGroup } from "./App.beb1f410.js";
...
function ListGroupItem(props) {
  var className = props.className, cssModule = props.cssModule, Tag = props.tag, active = props.active, disabled = props.disabled, action = props.action, color = props.color, attributes = _objectWithoutProperties$2(props, _excluded$1);
...
const getAudioData = (e2) => {
  ...
  var _a, _b, _c;
  const { channel: t2 = 0, samples: v = e2.length, height: h2 = 100, width: r2 = 800, top: s2 = 0, left: l = 0, type: i2 = "steps", paths: $ = [{ d: "Q", sx: 0, sy: 0, x: 50, y: 100, ex: 100, ey: 0 }], animation: o2 = false, animationframes: n2 = 10, normalize: b = true } = a2;
  var a2 = getFramesData(e2, t2, o2, n2), e2 = getFilterData(a2, v), w = b ? getNormalizeData(e2) : e2;
  let c = "";
  var p, f, g = i2 != "bars" ? (h2 + 2 * s2) / 2 : h2 + s2, m = r2 / v, y = $.length, u = i2 == "mirror" ? 2 * y : y, A = w.length;
  for (let n22 = 0; n22 < A; n22++) {
    0 < n22 && (p = c.length, f = c.charAt(p - 1), c += f == ";" || p === 0 ? " M 0 0 ;" : ";");
    let s22 = -9999, o22 = -9999;
    for (let r22 = 0; r22 < v; r22++) {
      var M = i2 == "bars" || r22 % 2 ? 1 : -1;
      let t22 = 1;
      for (let e22 = 0; e22 < u; e22++) {
        let a22 = e22;
        ...
        var d = $[a22].normalize ? 1 : w[n22][r22];
        if ($[a22].minshow <= w[n22][r22] && $[a22].maxshow >= w[n22][r22])
          switch ($[a22].d) {
            case "L":
              var D = r22 * m + m * $[a22].sx / 100 + l, x = g + d * $[a22].sy / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, k = r22 * m + m * $[a22].ex / 100 + l, z = g + d * $[a22].ey / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22;
              D === s22 && x === o22 || (c += `M ${D} ${x} `), c += `L ${k} ${z} `, s22 = k, o22 = z;
              break;
            case "H":
              D = r22 * m + m * $[a22].sx / 100 + l, x = g + d * $[a22].y / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, k = r22 * m + m * $[a22].ex / 100 + l;
              D === s22 && x === o22 || (c += `M ${D} ${x} `), c += `H ${k} `, s22 = k, o22 = x;
              break;
            case "V":
              var z = r22 * m + m * $[a22].x / 100 + l, C = g + d * $[a22].sy / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, F = g + d * $[a22].ey / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22;
              z === s22 && C === o22 || (c += `M ${z} ${C} `), c += `V ${F} `, s22 = z, o22 = F;
              break;
            case "C":
              var C = r22 * m + m * $[a22].sx / 100 + l, F = g - g * $[a22].sy / 100 * M, Q = r22 * m + m * $[a22].x / 100 + l, P = g + d * $[a22].y / 100 * (i2 != "bars" ? h2 : 2 * h2) * -M * t22, L = r22 * m + m * $[a22].ex / 100 + l, Z = g - g * $[a22].ey / 100 * M;
              C === s22 && F === o22 || (c += `M ${C} ${F} `), c += `C ${C} ${F} ${Q} ${P} ${L} ${Z} `, s22 = L, o22 = Z;
              break;
            case "Q":
              var Q = r22 * m + m * $[a22].sx / 100 + l, P = g + d * $[a22].sy / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, L = r22 * m + m * $[a22].x / 100 + l, Z = g + d * $[a22].y / 100 * (i2 != "bars" ? h2 : 2 * h2) * -M * t22, N = r22 * m + m * $[a22].ex / 100 + l, H = g + d * $[a22].ey / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22;
              Q === s22 && P === o22 || (c += `M ${Q} ${P} `), c += `Q ${L} ${Z} ${N} ${H} `, s22 = N, o22 = H;
              break;
            case "A": {
              var N = r22 * m + m * $[a22].sx / 100 + l, H = g + d * $[a22].sy / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, V = r22 * m + m * $[a22].ex / 100 + l, B = g + d * $[a22].ey / 100 * (i2 != "bars" ? h2 / 2 : h2) * -M * t22, I = (N === s22 && H === o22 || (c += `M ${N} ${H} `), $[a22].rx * m / 100), R = $[a22].ry * m / 100;
              let e3 = $[a22].sweep;
              M == -1 && (e3 = e3 == 1 ? 0 : 1), t22 == -1 && (e3 = e3 == 1 ? 0 : 1), c += `A ${I} ${R} ${$[a22].angle} ${$[a22].arc} ${e3} ${V} ${B} `, s22 = V, o22 = B;
              break;
            }
            case "Z":
              c += "Z ";
...
const i = { fetchBlob: function(e2, i2, s2) {
  var n2, r2;
  return t(this, void 0, void 0, function* () {
    const o2 = yield fetch(e2, s2);
    {
      const e3 = null === (n2 = o2.clone().body) || void 0 === n2 ? void 0 : n2.getReader(), s3 = Number(null === (r2 = o2.headers) || void 0 === r2 ? void 0 : r2.get("Content-Length"));
      let a2 = 0;
      const h2 = (n3, r3) => t(this, void 0, void 0, function* () {
        if (n3)
          return;
        a2 += (null == r3 ? void 0 : r3.length) || 0;
        const t2 = Math.round(a2 / s3 * 100);
        return i2(t2), null == e3 ? void 0 : e3.read().then(({ done: t3, value: e4 }) => h2(t3, e4));
      });
      null == e3 || e3.read().then(({ done: t2, value: e4 }) => h2(t2, e4));
    }
    return o2.blob();
  });
} };
...
class n extends s {
  constructor(t2) {
    super(), t2.media ? this.media = t2.media : this.media = document.createElement("audio"), t2.mediaControls && (this.media.controls = true), t2.autoplay && (this.media.autoplay = true), null != t2.playbackRate && this.onceMediaEvent("canplay", () => {
...
const a = { waveColor: "#999", progressColor: "#555", cursorWidth: 1, minPxPerSec: 0, fillParent: true, interact: true, autoScroll: true, autoCenter: true, sampleRate: 8e3 };
class h extends n {
  static create(t2) {
    return new h(t2);
  }
  ...
  loadAudio(s2, n2, r2, o2) {
    return t(this, void 0, void 0, function* () {
      if (this.emit("load", s2), this.isPlaying() && this.pause(), this.decodedData = null, this.duration = null, !n2 && !r2) {
        const t2 = (t3) => this.emit("loading", t3);
        n2 = yield i.fetchBlob(s2, t2, this.options.fetchParams);
      }
      if (this.setSrc(s2, n2), this.duration = (yield Promise.resolve(o2 || this.getDuration())) || (yield new Promise((t2) => {
        this.onceMediaEvent("loadedmetadata", () => t2(this.getDuration()));
      })) || (yield Promise.resolve(0)), r2)
        this.decodedData = e.createBuffer(r2, this.duration);
      else if (n2) {
        const t2 = yield n2.arrayBuffer();
        this.decodedData = yield e.decode(t2, this.options.sampleRate), 0 !== this.duration && this.duration !== 1 / 0 || (this.duration = this.decodedData.duration);
      }
  ...
  load(e2, i2, s2) {
    return t(this, void 0, void 0, function* () {
      this.loadAudio(e2, void 0, i2, s2);
    });
  }
  loadBlob(e2, i2, s2) {
    return t(this, void 0, void 0, function* () {
      this.loadAudio("blob", e2, i2, s2);
    });
  }
  zoom(t2) {
    if (!this.decodedData)
...
function WaveAudioInput({ file, setFile }) {
  ...
  react.exports.useEffect(() => {
    ...
    wavesurfer.current = h.create(options);
